...
def parse_lyric_credits(text):
    ...
    返回 {lyricist, composer, arranger, producer}
    """
    credits = {"lyricist": None, "composer": None, "arranger": None, "producer": None}
    if not text:
        return credits
...
class MusicMatcher:
    ...
    async def qq_search(self, session, keyword, limit=10):
        """搜索歌曲，返回标准化候选列表"""
        url = f"{self.qq_api_base}/getSearchByKey"
        data = await self._api_get(session, url, params={"key": keyword, "limit": limit, "page": 1})
    ...
    async def qq_lyric(self, session, songmid):
        ...
        url = f"{self.qq_api_base}/getLyric"
        data = await self._api_get(session, url, params={"songmid": songmid})
        response = data.get("response") if isinstance(data, dict) else data
        if isinstance(response, str):
            return None
    ...
    async def get_song_metadata(self, session, song_name, artist_name):
        ...
        if qq_item:
            result.update(
                song_name=qq_item.get("title") or song_name,
                artist=qq_item.get("artist") or artist_name,
                album=qq_item.get("album"),
                album_artist=qq_item.get("artist"),
            ...
            # 歌词解析四项
            if qq_item.get("songmid"):
                try:
                    lyric_text = await self.qq_lyric(session, qq_item["songmid"])
                    credits = parse_lyric_credits(lyric_text or "")
                    if credits["composer"]:
        ...
        if itunes_item and not qq_item:
            result.update(
                song_name=itunes_item.get("title") or song_name,
                artist=itunes_item.get("artist") or artist_name,
                album=itunes_item.get("album"),
                album_artist=itunes_item.get("album_artist"),
                year=itunes_item.get("year"),
                genre=itunes_item.get("genre"),
        ...
        # 3. MusicBrainz 兜底作曲 / 作词（歌词头缺失或为外文曲目时）
        if not result["composers"] or not result["lyricists"]:
    ...
    async def match_song(self, song_name, artist_name):
        async with aiohttp.ClientSession(timeout=self._timeout) as session:
            return await self.get_song_metadata(session, song_name, artist_name)
