...
import { FaArrowLeft, FaEdit, FaEllipsisH, FaExclamationCircle, FaFilm, FaPlay, FaTrash } from "react-icons/fa";

export default function VideoLibrary({ videos, albums, onOpen, onEdit, onDelete, title = "视频", onBack }) {
    ...
    {videos.length === 0 ? <div className="video-empty"><FaFilm size={38} /><p>暂无已入库视频</p><small>通过右上角“添加”导入本地视频或关联视频网站视频</small></div> : <div className="video-grid">
      {videos.map((video) => {
        ...
        return <article key={video.id} className={`video-card${isMissing ? " video-card-missing" : ""}`}>
          ...
          <div className="video-card-title-row"><button type="button" className="video-card-title" onClick={() => onOpen(video.id)}><strong>{video.title || "未命名视频"}</strong></button><div className="video-card-actions"><button type="button" className="video-card-more" aria-label="视频操作" onClick={() => setMenuVideoId((current) => current === video.id ? null : video.id)}><FaEllipsisH /></button>{menuVideoId === video.id && <div className="video-card-menu"><button type="button" onClick={() => { setMenuVideoId(null); onEdit?.(video.id); }}><FaEdit />更多信息</button><button type="button" className="danger" onClick={() => { setMenuVideoId(null); onDelete?.(video.id); }}><FaTrash />删除</button></div>}</div></div>
