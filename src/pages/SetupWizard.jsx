...
const initialState = {
  ...
  editMode: "default", editOptions: { publisher: true, keepArtist: true, hideArtist: true, collab: true },
...
export default function SetupWizard() {
  ...
  function renderContent() {
    ...
    if (step === 6) return <div className="setup-section"><ModeSelector value={data.editMode} onChange={(editMode) => set({ editMode })} />{data.editMode === "default" ? <DefaultSummary items={["发布者自动携带发布符号和日期", "保留无音乐的艺人", "默认隐藏空艺人", "自动整理合作艺人"]} /> : <ConfigBlock title="编辑与艺人设置">{EDIT_OPTIONS.map(([key, title, description]) => <ToggleRow key={key} title={title} description={description} checked={data.editOptions[key]} onChange={() => set({ editOptions: { ...data.editOptions, [key]: !data.editOptions[key] } })} />)}</ConfigBlock>}</div>;
